...
/// @brief init_game class
/// @details This class functions as the class which gets the input.
...
private:
    ...
    /// @brief keypad_listener member as reference.
    keypad_listener& keypad_listener;
    /// @brief DisplayControl member as reference.
    DisplayControl& displayControl;
    /// @brief game_parameters member as reference.
    game_parameters& gameParameters;
    /// @brief start_game member as reference.
    start_game& startGame;

    /// @brief Gun data member for storing gunpower.
    gun_data gun = {0,10}; //default
    /// @brief Time data member for storing playtime.
    tijd_countdown tijd_data = {12}; //default

    /// @brief restart_init_game_flag flag for starting restarting init game.
    rtos::flag restart_init_game_flag = {this, "restart_init_game_flag"};

    long long int delay = {rtos::ms * 200};

    /// @brief rtos timer for wrong input message.
    ...
    /// @brief Function for printing default messages for the menu.
    void print_ui(){
    ...
    /// @brief Function for clearing the default messages on the menu.
    void clear_ui(){
    ...
    /// @brief Function for getting the key which is pressed.
    /// @details The function checks if key A,B,C and D aren't pressed and it checks if * or # are pressed. It also checks if a number is pressed.
    int get_int(){
        displayControl.SetDisplayMessageD3("Input:\n");
        int n = 0;
        bool loop = true;
        while(loop){
            auto c = keypad_listener.getc();
            if(c != 'A' && c != 'B' && c != 'C' && c != 'D'){
                if(c == '*'){
                    n = 0;
                    displayControl.SetDisplayMessageD3("Input:\n");
    ...
    /// @brief Function which saves the given input as a player number.
    void input_number(){
        while (true){
            displayControl.SetDisplayMessageD2("Player number:\n(0-31)");
            gun.number = get_int();
    ...
    /// @brief Function which saves the given input as the game duration.
    void input_tijd(){
        while (true){
    ...
    /// @brief Function which saves the given input as the gun power.
    void input_gunpower(){
        while (true){
...
public:
    /// @b constructor for the init_game class
    /// @details This constructor initializes the reference members.
    /// @param gameParameters_ , a game_parameters class
    /// @param keypad_listener_ , a run_game class
    /// @param displayControl_ , a DisplayControl class
    /// @param startGame_ , a DisplayControl class
    init_game(keypad_listener& keypad_listener_, DisplayControl& displayControl_, game_parameters& gameParameters_, start_game& startGame_):
            keypad_listener(keypad_listener_),
    ...
    /// @brief Function restarts init game.
